...
public class SlotWheelView : MonoBehaviour {
    ...
    [Header("Parameters")]
    [SerializeField] private int _sequenceLength = 30;
    ...
    public async UniTask Spin(SymbolType targetSymbol, float duration) {
        _rollIndex = 0;
        _updatePointY = _scrollerInitialPos.y - _symbolHeight;
        ...
        await _scroller.DOMove(target, duration).OnUpdate(OnUpdate).ToUniTask();
